...
var instance = new axDrawer({
  content:
    '<aside class="console"id="console"><div class="csl"><div class="cslbody"><div class="mapselect ax-row row"><div class="text ax-col ax-col-6">地图:</div><div class="mapselectbody ax-col ax-col-18"><a id="mapselectbtn"class="ax-btn"data-selectedmap="brokenmoon">残月</a></div></div><div class="legendsselect ax-row row"><div class="texy ax-col ax-col-6">筛选:</div><div class="ax-col ax-col-18"><div id="checkbox"></div></div></div><div class="ax-row row"id="issue"><div class="text ax-col ax-col-6">Issue:</div><a href="https://github.com/Inkshow/apex-taisapoint/issues"class="ax-btn ax-col ax-col-9"target="_blank"><span class="ax-iconfont ax-icon-github"></span>Github</a><span class="ax-gutter-xs"></span><a href="https://gitee.com/S6equj5/apex-taisapoint/issues"class="ax-btn ax-col ax-col-9"target="_blank"><span class="ax-iconfont ax-icon-gitee"></span>Gitee</a></div></div></div></aside>',
  placement: "left",
});
function changeCslDisplay() {
  btn.style.left = "410px";
  btn.className = "ax-fadeInRight ax-slowest";
  instance.show();
}
instance.on("hide", function () {
  btn.style.left = "1.5rem";
  btn.className = "ax-fadeInLeft ax-slowest";
});
btn.addEventListener("click", changeCslDisplay);

//地图选择
let msltbtn = document.getElementById("mapselectbtn");
new axDropdown("#mapselectbtn", {
  type: "button",
  onBeforeSet: function (value) {
    if (value == "brokenmoon") {
      msltbtn.innerHTML = "残月";
    } else if (value == "olympus") {
      msltbtn.innerHTML = "奥林匹斯";
    } else {
      msltbtn.innerHTML = "世界尽头";
    }
    msltbtn.dataset.selectedmap = value;
    changeMap(value);
    this.setToHide();
  },
  content: [
    { value: "brokenmoon", label: "残月" },
    { value: "olympus", label: "奥林匹斯" },
    {
      value: "worldsedge",
      label: "世界尽头",
    },
...
//多选框
let data = [
  { value: "null", label: "无位移传奇" },
  { value: "Valkyrie", label: "瓦尔基里" },
  { value: "Ash", label: "艾许" },
  { value: "OthersDisplacement", label: "其他未经测试的位移传奇" },
];

let cbs = new axCheckbox("#checkbox", {
  children: data,
});
cbs.checkedAll();
cbs.on("change", (obj) => {
...
function analyze(a) {
  var imagenaturalWidth, imagenaturalHeight, mapclientWidth, mapclientHeight;
  mapimage.onload = function () {
    imagenaturalWidth = mapimage.naturalWidth;
    imagenaturalHeight = mapimage.naturalHeight;
    mapclientWidth = mapimage.clientWidth;
    mapclientHeight = mapimage.clientHeight;
    ...
    const infobody = document.getElementById("padding");
    ...
    for (var i = 0; i < svgpt.length; i++) {
      svgpt[i].addEventListener("pointerdown", function (e) {
        ...
        let infocontent = "";
        selectedmap =
          document.getElementById("mapselectbtn").dataset.selectedmap;
        if (svgp.dataset.fromvideo === undefined) {
          infocontent =
            '<a id="ifclose"><i class="ax-iconfont ax-icon-close"></i></a><div id="imgs">' +
        ...
        infobody.innerHTML = infocontent;
        //显示info
        info.style.display = "block";
        cover.style.display = "block";
        setTimeout(() => {
          info.className = "ax-scaleIn";
        ...
        //关闭info
        const closesvg = document.getElementById("ifclose");
        function closeif() {
          cover.style.opacity = "0";
          info.className = "ax-scaleOut";
          setTimeout(() => {
            info.style.display = "";
            cover.style.display = "";
          }, 202);
